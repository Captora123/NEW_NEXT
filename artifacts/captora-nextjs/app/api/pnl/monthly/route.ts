...
async function calcPnL(month: number, year: number) {
  ...
  return {
    month, year, totalRevenue, totalFreelancerCosts, totalSalaries, totalExpenses,
    netProfit: totalRevenue - totalFreelancerCosts - totalSalaries - totalExpenses,
  };
}
...
export async function GET(request: NextRequest) {
  ...
  return Response.json(await calcPnL(month, year));
}
