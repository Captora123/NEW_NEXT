    ...
    <div className="space-y-4">
              ...
              <div key={shoot.id} className="bg-white rounded-xl border border-slate-200 p-4 flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {shoot.functions && shoot.functions.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-1.5">
                      {shoot.functions.map(fn => <span key={fn} className="px-1.5 py-0.5 rounded text-[10px] font-semibold" style={{ background: `${ACCENT}18`, color: ACCENT }}>{fn}</span>)}
                    </div>
                  )}
                </div>
                <div className="flex flex-col items-end gap-2 flex-shrink-0">
                  ...
                  <div className="flex gap-1">
                    <button onClick={() => openEdit(shoot)} className="p-1.5 rounded-lg text-slate-300 hover:text-blue-600 hover:bg-blue-50"><Pencil className="w-3.5 h-3.5" /></button>
                    <button onClick={() => setDeleteId(shoot.id)} className="p-1.5 rounded-lg text-slate-300 hover:text-red-600 hover:bg-red-50"><Trash2 className="w-3.5 h-3.5" /></button>
                  </div>
                </div>
              </div>
            ))}
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-100">
                  {["Client", "Date & Time", "Location", "Service Type", "Status", "Actions"].map(h => (
                  ...
                  <tr key={shoot.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-5 py-4">
                      <div className="flex flex-wrap gap-1">
                        {shoot.functions?.map(fn => <span key={fn} className="px-2 py-0.5 rounded-full text-xs font-semibold border" style={{ background: `${ACCENT}12`, color: ACCENT, borderColor: `${ACCENT}30` }}>{fn}</span>)}
                    ...
                    <td className="px-5 py-4">
                      <div className="flex gap-1.5">
                        <button onClick={() => openEdit(shoot)} className="p-1.5 rounded-lg text-slate-400 hover:text-blue-600 hover:bg-blue-50"><Pencil className="w-4 h-4" /></button>
                        <button onClick={() => setDeleteId(shoot.id)} className="p-1.5 rounded-lg text-slate-400 hover:text-red-600 hover:bg-red-50"><Trash2 className="w-4 h-4" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-1.5">
              ...
              <textarea value={form.specialInstructions || ""} onChange={e => setForm(f => ({ ...f, specialInstructions: e.target.value }))}
                rows={3} placeholder="Any special notes for this shoot..." className="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm text-slate-700 focus:outline-none resize-none" />
            </div>
      ...
      <Dialog open={deleteId !== null} onOpenChange={() => setDeleteId(null)}>
        <DialogContent className="max-w-sm mx-3">
          ...
          <p className="text-slate-500 text-sm">This will permanently delete this shoot.</p>
          <div className="flex gap-3 pt-2">
