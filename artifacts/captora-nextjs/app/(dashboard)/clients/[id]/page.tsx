...
export default function ClientDetail() {
  ...
  const clientId = parseInt((params.id as string) || "0", 10);
  ...
  const handleSaveExpenses = () => {
    updateExpensesMut.mutate({ id: clientId, data: { albumCost: Number(expForm.albumCost), miscExpenses: Number(expForm.miscExpenses) } }, {
      onSuccess: () => { qc.invalidateQueries({ queryKey: clientQKey }); setExpOpen(false); toast({ title: "Expenses updated ✓" }); },
      onError: () => toast({ variant: "destructive", title: "Update failed" }),
    });
  };

  const openExpenses = () => { setExpForm({ albumCost: albumCost, miscExpenses: miscExpenses }); setExpOpen(true); };
    ...
    <div className="space-y-5">
        ...
        <div className="lg:col-span-2 space-y-5">
          <div className="bg-white rounded-xl border border-slate-200 p-6">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div><p className="text-xs font-semibold uppercase tracking-wider text-slate-400 flex items-center gap-1 mb-1"><Calendar className="w-3 h-3" />Wedding Date</p><p className="font-semibold text-slate-800">{client.weddingDate || "Not set"}</p></div>
              <div><p className="text-xs font-semibold uppercase tracking-wider text-slate-400 flex items-center gap-1 mb-1"><MapPin className="w-3 h-3" />Venue</p><p className="font-semibold text-slate-800">{client.venue || "Not set"}</p></div>
              <div><p className="text-xs font-semibold uppercase tracking-wider text-slate-400 mb-1">City</p><p className="font-semibold text-slate-800">{client.city || "Not set"}</p></div>
              <div><p className="text-xs font-semibold uppercase tracking-wider text-slate-400 flex items-center gap-1 mb-1"><IndianRupee className="w-3 h-3" />Package</p><p className="font-bold text-lg" style={{ color: CORAL }}>₹{packageAmt.toLocaleString("en-IN")}</p></div>
              {client.functions?.length ? (
                <div className="col-span-2">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {client.functions.map(fn => <span key={fn} className="px-2.5 py-1 rounded-full text-xs font-semibold bg-orange-50 text-[#E0533C] border border-orange-200">{fn}</span>)}
                  </div>
                </div>
              ) : null}
            </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-6">
            ...
            <div className="grid grid-cols-3 gap-3 mb-4">
              <div className="p-4 rounded-xl bg-slate-50 border border-slate-200"><p className="text-xs text-slate-400 font-semibold uppercase tracking-wider">Package</p><p className="text-lg font-bold text-slate-800 mt-1">₹{packageAmt.toLocaleString("en-IN")}</p></div>
              <div className="p-4 rounded-xl bg-green-50 border border-green-200"><p className="text-xs text-green-600 font-semibold uppercase tracking-wider">Paid</p><p className="text-lg font-bold text-green-700 mt-1">₹{totalPaid.toLocaleString("en-IN")}</p></div>
              <div className="p-4 rounded-xl bg-red-50 border border-red-200"><p className="text-xs text-red-500 font-semibold uppercase tracking-wider">Pending</p><p className="text-lg font-bold text-red-600 mt-1">₹{pending.toLocaleString("en-IN")}</p></div>
            </div>
            {payments?.length ? (
              <div>
                {payments.map(p => (
          ...
          {packageAmt > 0 && (
            <div className="bg-white rounded-xl border border-slate-200 p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={openExpenses} className="text-xs font-semibold px-2.5 py-1.5 rounded-lg border border-slate-200 text-slate-500 hover:border-[#E0533C] hover:text-[#E0533C] transition-colors">Edit Costs</button>
              </div>
              <div className="space-y-2 mb-4">
                {[
                  { label: "Package Value", value: packageAmt, sign: "" },
                  { label: "Freelancer Costs", value: freelancerSpend, sign: "−", icon: <UserCheck className="w-3.5 h-3.5 text-slate-400" /> },
                  { label: "Album Cost", value: albumCost, sign: "−" },
                  { label: "Misc Expenses", value: miscExpenses, sign: "−" },
                ].map(({ label, value, sign, icon }) => (
                  <div key={label} className="flex justify-between items-center py-2.5 border-b border-slate-100 text-sm">
                    <span className="text-slate-600 flex items-center gap-1.5">{icon}{label}</span>
                    <span className={`font-${sign ? "semibold text-red-600" : "bold text-slate-900"}`}>{sign} {fmt(value)}</span>
                  </div>
                ))}
              </div>
              <div className={`flex items-center justify-between p-4 rounded-xl border-2 ${netProfit >= 0 ? "border-green-200 bg-green-50" : "border-red-200 bg-red-50"}`}>
                <div className="flex items-center gap-2">
                  {netProfit >= 0 ? <TrendingUp className="w-5 h-5 text-green-600" /> : <TrendingDown className="w-5 h-5 text-red-600" />}
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setAddFLOpen(true)} className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-white text-xs font-semibold" style={{ background: CORAL }}>
                ...
                {assignments.map((a: ClientFreelancerAssignment) => (
                  <div key={a.id} className="flex items-center justify-between p-3.5 rounded-xl bg-slate-50 border border-slate-100">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <button onClick={() => setDeleteAssignId(a.id)} className="p-1.5 rounded-lg text-slate-300 hover:text-red-600 hover:bg-red-50 transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
                    </div>
                  </div>
                ))}
        ...
        <div>
          <div className="bg-white rounded-xl border border-slate-200 p-6 sticky top-4">
            ...
            <div className="space-y-2 mb-4 max-h-72 overflow-y-auto">
              {loadingNotes ? <p className="text-sm text-slate-400">Loading...</p>
                : notes?.length ? [...notes].reverse().map(note => (
                  <div key={note.id} className="p-3 bg-slate-50 rounded-xl border border-slate-100">
                    ...
                    <p className="text-xs text-slate-400 mt-1.5">{new Date(note.createdAt).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" })}</p>
                  </div>
                )) : <p className="text-sm text-slate-400">No notes yet.</p>}
            </div>
            <div className="space-y-2">
              <Input value={noteText} onChange={e => setNoteText(e.target.value)} placeholder="Add a note..."
                onKeyDown={e => { if (e.key === "Enter") handleAddNote(); }} className="bg-slate-50 border-slate-200 text-slate-700" />
              <button onClick={handleAddNote} disabled={addNoteMut.isPending || !noteText.trim()}
                className="w-full py-2 rounded-lg text-white text-sm font-semibold disabled:opacity-50" style={{ background: CORAL }}>Add Note</button>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-1.5">
              ...
              <Select value={String(flForm.freelancerId || "")} onValueChange={v => setFlForm(f => ({ ...f, freelancerId: Number(v) }))}>
                ...
                <SelectContent>{allFreelancers?.map(f => <SelectItem key={f.id} value={String(f.id)}>{f.name} {f.role ? `(${f.role})` : ""}</SelectItem>)}</SelectContent>
              </Select>
            </div>
            <div className="space-y-1.5"><Label>Function *</Label><Input value={flForm.functionName} onChange={e => setFlForm(f => ({ ...f, functionName: e.target.value }))} placeholder="e.g. Mehendi, Baraat" /></div>
            <div className="space-y-1.5"><Label>Rate for This Shoot (₹)</Label><Input type="number" value={flForm.rateForShoot || ""} onChange={e => setFlForm(f => ({ ...f, rateForShoot: Number(e.target.value) }))} /></div>
            <div className="space-y-1.5"><Label>Notes</Label><Input value={flForm.notes} onChange={e => setFlForm(f => ({ ...f, notes: e.target.value }))} /></div>
            <div className="flex gap-3 pt-2">
              <button onClick={handleAddAssignment} disabled={addAssignmentMut.isPending}
                className="flex-1 py-2.5 rounded-lg text-white text-sm font-semibold disabled:opacity-60" style={{ background: CORAL }}>Assign Freelancer</button>
      ...
      <Dialog open={expOpen} onOpenChange={setExpOpen}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-1.5"><Label>Album Cost (₹)</Label><Input type="number" value={expForm.albumCost || ""} onChange={e => setExpForm(f => ({ ...f, albumCost: Number(e.target.value) }))} /></div>
            <div className="space-y-1.5"><Label>Misc Expenses (₹)</Label><Input type="number" value={expForm.miscExpenses || ""} onChange={e => setExpForm(f => ({ ...f, miscExpenses: Number(e.target.value) }))} /></div>
            <div className="flex gap-3 pt-2">
              <button onClick={handleSaveExpenses} disabled={updateExpensesMut.isPending}
                className="flex-1 py-2.5 rounded-lg text-white text-sm font-semibold disabled:opacity-60" style={{ background: CORAL }}>Save</button>
