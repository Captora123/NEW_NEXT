...
export default function Profit() {
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="flex gap-2 items-center">
          <div className="relative">
            <select value={month} onChange={e => setMonth(Number(e.target.value))}
              className="appearance-none bg-white border border-slate-200 text-sm font-medium px-4 py-2.5 pr-8 rounded-xl text-slate-700 focus:outline-none cursor-pointer shadow-sm">
      ...
      {isLoading ? (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">{[1, 2, 3, 4].map(i => <div key={i} className="h-28 bg-slate-100 animate-pulse rounded-xl" />)}</div>
      ) : (
