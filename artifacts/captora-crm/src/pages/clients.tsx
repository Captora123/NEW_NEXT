      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="sm:hidden space-y-2">
                ...
                <div className="bg-white rounded-xl border border-slate-200 p-4 flex items-center gap-3 active:bg-slate-50 transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      ...
                      <span className={`px-2 py-0.5 rounded-full text-xs font-semibold flex-shrink-0 ${STATUS_COLORS[client.status] || "bg-slate-100 text-slate-600"}`}>
                        {client.status}
                      </span>
                    </div>
                  ...
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    ...
                    <button onClick={e => { e.preventDefault(); setDeleteId(client.id); }} className="p-1.5 rounded-lg text-slate-300 hover:text-red-600 hover:bg-red-50 transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
            ...
            {filtered.length === 0 && (
              <div className="py-12 text-center text-slate-400 text-sm bg-white rounded-xl border border-slate-200">No clients found.</div>
            )}
          </div>
              ...
              <tbody>
                {filtered.map((client) => (
                  <tr key={client.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-5 py-4 text-sm text-slate-600 hidden md:table-cell">{client.weddingDate || "—"}</td>
                    <td className="px-5 py-4 text-sm font-semibold text-slate-800">₹{(client.packageAmount ?? 0).toLocaleString("en-IN")}</td>
                    <td className="px-5 py-4 text-sm font-semibold text-red-600 hidden md:table-cell">₹{(client.totalPending ?? 0).toLocaleString("en-IN")}</td>
                    <td className="px-5 py-4">
